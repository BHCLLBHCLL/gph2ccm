...
但**物理与求解层**（结果场、边界条件细节、材料属性、MRF 旋转条件、周期
配对、2D 包裹、质量修复）**基本缺失**，距离「求解器就绪导出器」还有明显差距。
...
| 11 | Verify 读回校验 | ⚠️ | split 模式因 interface 两侧共享面 **误报重复**（H1） |
| 12 | Multiple Processors（分布式） | ❌ | 单 processor |
| 13 | Solution Fields（结果场） | ❌ | 不写任何场数据 |
| 14 | Boundary Conditions（含对称 / 周期配对） | ❌ | 仅 boundary type 标签 |
| 15 | Material / Region 求解属性 | ❌ | 仅 fluid / solid 标签 |
...
**完整度统计**：21 项中已实现 11、部分 2、缺失 8 → **核心网格层基本覆盖，
物理 / 求解层几乎空白**。
...
1. **无结果场 / 求解设置**（最大功能缺口）—— 当前产物仅「几何 + 分区 + 边界类型」。
2. **边界条件仅类型名** —— 对称 / 周期 / 旋转的附加定义缺失，STAR-CCM+ 侧需大量手工补。
3. **MRF 仅 region 划分** —— 非真正的旋转参考系条件。
4. **周期 / 滑移界面配对** —— 识别了类型却未生成配对面。
5. **多 processor / 分布式** —— 不支持大网格并行分区写入。
6. **2D 网格包裹** —— 不支持 2D 算例。
7. **网格质量修复** —— 仅诊断，不修退化面 / 悬挂节点。
8. **测试与 CI 薄弱** —— 质量靠手动导入验证，回归风险高。

---

## 6. 建议路线

- **短期（保质量，低成本）**：修复 H1 / M1 / M4；补 `split_regions` 模式下
  `verify` 的接口面唯一性放宽逻辑，并加对应单测。
- **中期（扩广度）**：结果场写入（若上游 GPH 提供）、周期边界配对、
