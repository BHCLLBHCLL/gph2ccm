...
| 12 | gphdecoding | **GPH 网格无结果场**；同体系 **FPH 文件（魔数 `CRDL-FLD`）携带 FlowSolution**（`fph2cgns.py` 已解析，需 h5py）；向量以 `<base>X/Y/Z` 分量命名、哨兵值 >1e20 | C2 已落地：`gph2ccm/fph_fields.py` 自动分组标量/向量 + 哨兵清零；CLI 支持 `.fph` 输入与 `--fph` 附加结果文件 |
